 ...
 * Canonical 60-image pool supplied by the GEI project owner.
 * Designed to be consumed by index.html's V1.5 splash runtime.
 */
window.GEI_V16_SPLASH_ASSETS = Object.freeze([
...
window.GEI_V16_SPLASH_ROTATION = Object.freeze({
  version: "1.6.0",
  assetCount: window.GEI_V16_SPLASH_ASSETS.length,
  lastAssetKey: "gei_splash_last_asset_v1",
  recentAssetsKey: "gei_splash_recent_assets_v1",
  recentWindow: 12,
  select(pool = window.GEI_V16_SPLASH_ASSETS) {
    const list = Array.from(new Set(pool.filter(Boolean)));
    if (!list.length) return "";
    const last = localStorage.getItem(this.lastAssetKey) || "";
    let recent = [];
    try { recent = JSON.parse(localStorage.getItem(this.recentAssetsKey) || "[]"); } catch (_) { recent = []; }
    const preferred = list.filter(asset => asset !== last && !recent.includes(asset));
    const candidates = preferred.length ? preferred : list.filter(asset => asset !== last);
  ...
  commit(asset) {
    ...
    let recent = [];
    try { recent = JSON.parse(localStorage.getItem(this.recentAssetsKey) || "[]"); } catch (_) { recent = []; }
    recent = [asset, ...recent.filter(item => item !== asset)].slice(0, this.recentWindow);
    try {
      localStorage.setItem(this.lastAssetKey, asset);
      localStorage.setItem(this.recentAssetsKey, JSON.stringify(recent));
    } catch (_) {}
  }
});
