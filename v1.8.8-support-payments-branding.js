 ...
 * Adds PayPal and Cash App support actions and defensively restores the
 * official YallToo logo where the current shell expects it.
 */
(function(){
  'use strict';
  function boot(){
    ...
    css.textContent=''+
      '.gei-support-payments{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:12px}'+
      '.gei-support-pay{display:flex;align-items:center;justify-content:center;min-height:48px;padding:10px 12px;border-radius:14px;text-decoration:none;color:#fff;font-weight:800;font-size:.76rem;letter-spacing:.02em;text-align:center;box-shadow:0 8px 18px rgba(0,0,0,.12);transition:transform .2s ease,filter .2s ease}'+
      '.gei-support-pay:active{transform:scale(.97);filter:brightness(.96)}'+
      '.gei-support-pay.paypal{background:linear-gradient(135deg,var(--indigo),var(--cyan))}'+
      '.gei-support-pay.cashapp{background:linear-gradient(135deg,var(--magenta),var(--pink))}'+
      '.gei-support-pay small{display:block;font-size:.56rem;font-weight:700;opacity:.85;margin-top:2px}'+
      '@media(max-width:350px){.gei-support-payments{grid-template-columns:1fr}.gei-support-pay{min-height:44px}}';
    document.head.appendChild(css);

    var logo='https://assets.zyrosite.com/YZ9jg46Bljs5wOZR/yall-too-logo-vq2GKHxbTzr7g8bO.png';
    document.querySelectorAll('.gei-yalltoo-logo').forEach(function(img){
